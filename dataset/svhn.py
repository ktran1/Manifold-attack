...
svhn_mean = (0.4914, 0.4822, 0.4465) # equals np.mean(train_set.train_data, axis=(0,1,2))/255
svhn_std = (0.2471, 0.2435, 0.2616) # equals np.std(train_set.train_data, axis=(0,1,2))/255

def normalise(x, mean=svhn_mean, std=svhn_std):
    x, mean, std = [np.array(a, np.float32) for a in (x, mean, std)]
    x -= mean[None,:,None,None]*255
    x *= 1.0/(255*std[None,:,None,None])
    return x
